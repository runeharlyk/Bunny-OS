<script lang="ts">
    ...
    function minimize(){ process.minimized = true }
...
<div class="touch-none resize-none hover:resize w-1/3 h-1/3 absolute select-none transition-[transform,opacity]" 
    style="width:{fullScreen?window.screen.availWidth:width}px; height:{fullScreen?window.screen.availHeight:height}px; left: {fullScreen?0:left}px; top: {fullScreen?0:top}px; transform: translate({process.minimized?`calc(100% - ${left}px)`:"0"}, {process.minimized?`calc(80vw - ${top}px)`:"0"}); opacity:{process.minimized?0:1}">
    <Resizer bind:width={width} bind:height={height} bind:top_ab={top} bind:left_ab={left}/>
